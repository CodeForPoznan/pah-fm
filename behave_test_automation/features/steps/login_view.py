...
def visibility_of_fields(context, field_name) -> None:
    fields = {
        'username_field': login_selector.username_field,
        'password_field': login_selector.password_field,
    }
...
def step_impl(context, text) -> None:
    context.driver.find_element(*login_selector.login_button_enabled)
...
def click_login(context) -> None:
    login_button_enabled = context.driver.find_element(*login_selector.login_button_enabled)
    login_button_enabled.click()
    short_wait(context.driver, main_menu_selector.hamburger_menu)


@step("User is on add new drive page")
def drive_page_view(context) -> None:
